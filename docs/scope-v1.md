...
| 6 | Live events | **List-and-link model.** Clerics stream on their existing platforms (TikTok/YouTube/etc.); we host the event schedule and the talisman link gets pinned in stream comments/bio. We do NOT build our own streaming. **SUPERSEDED 2026-09-14 by `docs/scope-v2-live-events.md`** — Zahir moved to the next phase. Streaming is now BOTH, in phases: external link-out (this decision's original shape) plus in-app hosted media later, not started. v2 covers the companion-page layer only — schedule, live status, a public/anonymous question queue, and a 1-on-1 booking queue charged only when the speaker actually sees the attendee. Built as a working prototype (placeholders for payment/backend/login/hosting) and verified end-to-end 2026-09-15; not yet merged to `main`, not pushed to `origin/main`. | Zahir agreed with General's recommendation, 2026-08-21 |
