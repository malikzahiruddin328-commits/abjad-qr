 ...
 * Ready to integrate with Gmail MCP, SendGrid, or Mailgun
 *
 * Usage:
 * 1. For Gmail MCP: Uncomment GMAIL_MCP section
 * 2. For SendGrid: Uncomment SENDGRID section + set SENDGRID_API_KEY
 * 3. For Mailgun: Uncomment MAILGUN section + set MAILGUN_API_KEY
 */
...
const EMAIL_CONFIG = {
  ...
  // Gmail MCP (recommended for testing with existing Gmail account)
  GMAIL_ACCOUNT: 'support@babaji.com', // Change to your Gmail
...
// ============================================================
// GMAIL MCP (Recommended - uses your existing Gmail account)
// ============================================================
async function sendViaGmailMCP(to, templateName, data) {
  // Requires Gmail MCP to be connected
  // This will be called from the backend/frontend with Gmail MCP available
  ...
  try {
    // This would be handled by the Gmail MCP connector
    // For now, log it
    ...
    //   subject: emailData.subject,
    //   html: emailData.html,
    //   from: EMAIL_CONFIG.GMAIL_ACCOUNT
    // });
...
async function sendEmail(to, templateName, data) {
  ...
  // Priority order: Gmail MCP > SendGrid > Mailgun > Console

  // Try Gmail MCP first (recommended)
...
async function sendEmailBatch(recipients, templateName, data) {
  ...
  return results.every(r => r); // True if all succeeded
}
